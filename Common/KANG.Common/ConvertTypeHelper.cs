...
namespace KANG.Common {
    public static class ConvertTypeHelper {
        ...
        public static string CharArray2String(char[] in_str) {
            var out_str = new string(in_str);
            var i = out_str.IndexOf('\0', 0);
            if (i == -1)
                i = 16;
        ...
        public static string ByteArray2String1(byte[] in_str) {
            var out_str = Encoding.Default.GetString(in_str);
        ...
        public static byte[] String2ByteArray(string in_str, int iLen) {
            ...
            var bsources = Encoding.Default.GetBytes(in_str);
        ...
        /// <returns></returns>
        public static byte[] Image_Image2Byte(Image pic, ImageFormat fmt) {
        ...
        public static Image Image_Byte2Image(byte[] bytes) {
            var mem = new MemoryStream(bytes, true);
            mem.Read(bytes, 0, bytes.Length);
            mem.Flush();
            var aa = Image.FromStream(mem);
            return aa;
        }
        ...
        public static string ByteArray2String(byte[] bytes) {
            var strSignalData = BitConverter.ToString(bytes).Replace("-", ",");
            var hexValuesSplit = strSignalData.Split(',');
            // string[] stringValues = new string[hexValuesSplit.Length];
            var strValues = "";
            for (var i = 0; i < hexValuesSplit.Length; i++) {
                var value = Convert.ToInt32(hexValuesSplit[i], 16);
        ...
        public static string HexValueArray2String(string[] hexArr) {
            var strValues = "";
            var chars = new char[hexArr.Length];
