...
namespace KANG.Common {
    public class PictureHelper {
        ...
        private class MyFont {
            ...
            private Bitmap Bmap { get; }
            private string Content { get; }
            private Color TextColor { get; }
            private float TextFontSize { get; }
            private ScreenStyle TextStyle { get; }
                ...
                switch (TextStyle) {
                    case ScreenStyle.UpAndDown: {
                        if (Content.Length <= 30) {
                            ...
                            //30~60字时，再绘制一行
                            var xPos = (Bmap.Width - TextFontSize*30)/2;
                            g.DrawString(Content.Substring(0, 30), drawFont, drawBrush, xPos, pos["yPos"]);
                            g.DrawString(Content.Substring(30), drawFont, drawBrush, xPos,
                                pos["yPos"] + TextFontSize + 10);
                        }
                    }
                        break;
                    case ScreenStyle.LeftAndRight:
                    case ScreenStyle.FullScreen: {
                        ...
                        switch ((Content.Length - 1)/15) //长度减1，防止当文字刚好为60时不显示
                        {
                            ...
                            case 1: {
                                ...
                                g.DrawString(Content.Substring(15), drawFont, drawBrush, pos["xPos"],
                                    pos["yPos"] + TextFontSize + 10);
                            }
                                break;
                            case 2: {
                                g.DrawString(Content.Substring(0, 15), drawFont, drawBrush, pos["xPos"], pos["yPos"]);
                                g.DrawString(Content.Substring(15, 15), drawFont, drawBrush, pos["xPos"],
                                    pos["yPos"] + TextFontSize + 10);
                                g.DrawString(Content.Substring(30), drawFont, drawBrush, pos["xPos"],
                                    pos["yPos"] + (TextFontSize + 10)*2);
                            }
                                break;
                            case 3: {
                                g.DrawString(Content.Substring(0, 15), drawFont, drawBrush, pos["xPos"], pos["yPos"]);
                                g.DrawString(Content.Substring(15, 15), drawFont, drawBrush, pos["xPos"],
                                    pos["yPos"] + TextFontSize + 10);
                                g.DrawString(Content.Substring(30, 15), drawFont, drawBrush, pos["xPos"],
                                    pos["yPos"] + (TextFontSize + 10)*2);
                                g.DrawString(Content.Substring(45), drawFont, drawBrush, pos["xPos"],
                                    pos["yPos"] + (TextFontSize + 10)*3);
                            }
                                break;
                        }
                    }
