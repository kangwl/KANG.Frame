...
namespace KANG.Common.Sockets
{
    ...
    public class AsyncSocketTcpServer : MarshalByRefObject, IDisposable
    {
        ...
        public bool Start(bool throwOnError = false)
        {
            ...
            if (!this.IsListening)
            {
                try
                {
                    ...
                    this._listenSocket.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.NoDelay, true);
        ...
        private void CheckDisposed()
        {
            if (this._disposed)
            {
                throw new ObjectDisposedException("DevLib.Net.Sockets.AsyncSocketTcpServer");
            }
        }
    }
}
