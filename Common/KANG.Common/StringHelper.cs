...
namespace KANG.Common {
    public class StringHelper {
        ...
        /// </summary>
        public static string[] MonthesEng {
            get {
                return new[] {
        ...
        public static string GetSubString(string p_SrcString, int p_StartIndex, int p_Length,
            string p_TailString = "...") {
            var myResult = p_SrcString;

            var bComments = Encoding.UTF8.GetBytes(p_SrcString);
            ...
            if (p_Length >= 0) {
                var bsSrcString = Encoding.Default.GetBytes(p_SrcString);

                //当字符串长度大于起始位置
                if (bsSrcString.Length > p_StartIndex) {
                    var p_EndIndex = bsSrcString.Length;
                    ...
                    var nRealLength = p_Length;
                    var anResultFlag = new int[p_Length];
                    byte[] bsResult;

                    var nFlag = 0;
                    for (var i = p_StartIndex; i < p_EndIndex; i++) {
