...
namespace KANG.Common.web {
    public class HttpHelper {
        ...
        public static HttpWebResponse CreatePostHttpResponse(string url, IDictionary<string, string> parameters,
            int? timeout, string userAgent, Encoding requestEncoding, CookieContainer cookies) {
            if (string.IsNullOrEmpty(url)) {
                throw new ArgumentNullException("url");
            }
        ...
        public static string GetPage(string url, Encoding Ec) {
            ...
            try {
                ...
                using (var stream = webResponse.GetResponseStream()) {
                    using (var reader2 = new StreamReader(stream, Ec)) {
        ...
        public static string GetHttpFile(string sUrl, string sSavePath, string fileExtension) {
            ...
            var fullName = Guid.NewGuid().ToString("n");
            var fileName = savePath + fullName + fileExtension;
            string bRslt;
            WebResponse oWebRps = null;
            var oWebRqst = WebRequest.Create(sUrl);
                ...
                if (!ifExisit) {
                    ...
                    if (oWebRps != null)
                        using (
                            ...
                            var iLen = Convert.ToInt32(oWebRps.ContentLength);
                            try {
        ...
        public static string GetWebString(string reqJonUrl) {
            ...
            try {
                var uri = new Uri(reqJonUrl);
                var stream = WebRequest.Create(uri).GetResponse().GetResponseStream();
