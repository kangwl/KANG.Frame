...
(function (c) {
                ...
                if (e[0]) {
                    if (e[0] == "*") {
                        ...
                        c("#" + j.queueID).find(".uploadify-queue-item").each(function () {
                            ...
                            c(this).find(".data").removeClass("data").html(" - 已取消");
                        ...
                        for (var m = 0; m < e.length; m++) {
                            i.cancelUpload(e[m]);
                            c("#" + e[m]).find(".data").removeClass("data").html(" - 已取消");
    ...
    var a = {
        onDialogOpen: function () {
            var d = this.settings;
            this.queueData.errorMsg = "有些文件没有添加到队列中:";
        ...
        onSelect: function (h) {
            ...
            for (var g in this.queueData.files) {
                ...
                if (f.uploaded != true && f.name == h.name) {
                    var e = confirm('该文件 "' + h.name + '" 已经在队列中.\n你想替换队列中的文件吗?');
                    if (!e) {
                ...
                switch (g) {
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        if (e.queueSizeLimit > f) {
                            this.queueData.errorMsg += "\n选定的文件的数量超过了剩余的上传限制 (" + f + ").";
                        } else {
                            this.queueData.errorMsg += "\n选定的文件的数目超过了队列大小限制 (" + e.queueSizeLimit + ").";
                        }
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n该文件 "' + d.name + '" 超过了大小限制 (' + e.fileSizeLimit + ").";
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\n该文件 "' + d.name + '" 是空文件.';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n该文件 "' + d.name + '" 不是允许的上传类型 (' + e.fileTypeDesc + ").";
                        break;
                }
            }
